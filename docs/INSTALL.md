...
```bash
# Pin a version.
curl -fsSL .../install.sh | bash -s -- --version v0.37.0
...
& ([scriptblock]::Create($installer)) -Version v0.37.0 -WithClaudeConfig
```
...
cargo install --locked --git https://github.com/agustincbajo/Coral --tag v0.37.0 coral-cli
```
...
```bash
coral self-upgrade                    # default: latest same-major (v0.37.x -> v0.37.y)
coral self-upgrade --check-only       # report-only; never mutate
coral self-upgrade --version v0.37.1  # pin to a specific same-major release
```
...
- **Use the Coral composite actions** in your `.github/workflows/wiki.yml`:
  ```yaml
  - uses: agustincbajo/Coral/.github/actions/ingest@v0.37.0
    with:

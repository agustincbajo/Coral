...
- `--fix` (v0.12, `confidence-from-coverage` added in v0.14): no-LLM mechanical rule auto-fix. Trim trailing whitespace in frontmatter, sort+dedup `sources`/`backlinks`, normalize `[[ slug ]]`→`[[slug]]`, normalize CRLF→LF, trim trailing line whitespace, **and** downgrade `confidence` by 0.20 (floored at 0.30) when any entry in `sources:` no longer resolves under the repo root (calibration signal — backs the same "missing source" case that `SourceNotFound` already lints). Default dry-run; `--apply` writes. Composes with `--auto-fix`.
